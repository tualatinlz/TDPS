...
#define MPU_I2C_DRCECT_TRNT 		0x00
#define MPU_I2C_DRCECT_RECV 		0x01

u8 AnBT_DMP_I2C_Write(u8 anbt_dev_addr, u8 anbt_reg_addr, u8 anbt_i2c_len, u8 *anbt_i2c_data_buf)
{		
	u8 i;
	MPU_I2C_Start();
	MPU_I2C_SendByte(anbt_dev_addr << 1 | MPU_I2C_DRCECT_TRNT);				//发送陀螺仪写地址
	while(!MPU_I2C_WaitAck());	
	MPU_I2C_SendByte(anbt_reg_addr);  			//发送陀螺仪PWM地址
	while(!MPU_I2C_WaitAck());
	for (i=0;i<anbt_i2c_len;i++) 
	{
		MPU_I2C_SendByte(anbt_i2c_data_buf[i]); //发送陀螺仪PWM值
		while(!MPU_I2C_WaitAck());
	}
	MPU_I2C_Stop();
	return 0x00;
...
u8 AnBT_DMP_I2C_Read(u8 anbt_dev_addr, u8 anbt_reg_addr, u8 anbt_i2c_len, u8 *anbt_i2c_data_buf)
{
	...
	MPU_I2C_SendByte(anbt_dev_addr << 1 | MPU_I2C_DRCECT_TRNT);			//发送陀螺仪写地址
	while(!MPU_I2C_WaitAck());
	MPU_I2C_SendByte(anbt_reg_addr);  																		//发送陀螺仪ID地址
	while(!MPU_I2C_WaitAck());
	MPU_I2C_Start();
	MPU_I2C_SendByte(anbt_dev_addr << 1 | MPU_I2C_DRCECT_RECV);       //发送陀螺仪读地址
	while(!MPU_I2C_WaitAck());

	while (anbt_i2c_len) 
	{
		if (anbt_i2c_len==1) 
		{	*anbt_i2c_data_buf =MPU_I2C_ReadByte();	MPU_I2C_NoAck();}  
		else 
		{ *anbt_i2c_data_buf =MPU_I2C_ReadByte();	MPU_I2C_Ack();	} 
		anbt_i2c_data_buf++;
		anbt_i2c_len--;
	}
	MPU_I2C_Stop();
